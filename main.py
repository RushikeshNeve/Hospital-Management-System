...
def index():
    if "user" in session:
        if session["user"] == "Admin":
            ...
            cursor.execute("SELECT * FROM appointment order by appointmentdate")
...
def deletedoctor(doctorid):
    if "user" in session:
        if session["user"] == "Admin":
            cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
            cursor.execute("DELETE from doctor where doctorid='%d'" %(doctorid))
            mysql.connection.commit()
            return redirect('/doctors')
        else :
            return redirect("/logout")
...
def editappointment(appointmentid):
    if "user" in session:
        if session["user"] == "Admin":
            ...
            row=cursor.execute("SELECT * FROM appointment where appointmentid='%d'" %appointmentid)
            row=cursor.fetchone()
            if row['status'] == "Active":
                cursor.execute("UPDATE appointment SET status='%s' WHERE appointmentid='%s'" %("Inactive",(appointmentid)))
            else:
                cursor.execute("UPDATE appointment SET status='%s' WHERE appointmentid='%s'" %("Active",(appointmentid)))
            mysql.connection.commit()
    ...
    if "user" in session:
        if session["user"] == "Admin":
            if request.method == 'POST':
                ...
                cursor.execute("INSERT INTO patient (patientname,email,address, mobileno,city,pincode,bloodgroup,gender,age,illness) VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')" %(name, email,address,phone,city,pincode,blood_group,gender,age,illness))
                mysql.connection.commit()
...
def deletepatient(patientid):
    if "user" in session:
        if session["user"] == "Admin":
            cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
            cursor.execute("DELETE from patient where patientid='%d'" %(patientid))
            mysql.connection.commit()
            return redirect('/patients')
        else :
            return redirect("/logout")
...
def invoices_bill():
    if "user" in session:
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute("SELECT * FROM  billing where patientname='%s'" %session['username'])
        row=cursor.fetchone()
        cursor.execute("SELECT * FROM  appointment where patientname='%s'" %row['patientname'])
        patientname=cursor.fetchone()
        cursor.execute("SELECT * FROM  doctor where doctorname='%s'" %patientname['doctorname'])
